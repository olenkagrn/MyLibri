...
form.addEventListener("submit", async function (event) {
  ...
  try {
    ...
    if (response.ok) {
      openModal(); // Відкриваємо модальне вікно після успішного додавання
      form.reset(); // Очищаємо форму

      // Очищення поля файлу
      inputFile.value = "";
      imageView.style.backgroundImage = ""; // Прибираємо попередній фон
      imageView.innerHTML = `
      ...
      `;
      imageView.style.border = ""; // Повертаємо стандартний стиль
    } else {
